    ...
    <div className="min-h-screen bg-surface">
      <div className="max-w-2xl mx-auto px-4 py-6 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          <Link href="/profil" className="text-sm text-bleu-moyen hover:text-bleu-profond transition-colors">
            ← Mon compte
          </Link>
            ...
            {avis.map((a) => {
              ...
              return (
                <div key={a.id} className="bg-white rounded-2xl border border-border p-4 flex flex-col gap-3">
                  ...
                  {/* Actions — masquées quand le formulaire est ouvert */}
                  {avisEnEdition !== a.id && (
                    <div className="flex gap-2 pt-1">
                      <button
                        onClick={() => setAvisEnEdition(a.id)}
                        className="flex-1 py-2 rounded-xl border border-bleu-profond
                                   text-bleu-profond text-xs font-semibold
                      ...
                      <button
                        ...
                        className="flex-1 py-2 rounded-xl border border-rouge
                                   text-rouge text-xs font-semibold
                                   hover:bg-rouge/10 transition-colors
                                   disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {suppressionEnCours === a.id ? "Suppression..." : "🗑️ Supprimer"}
                      </button>
                    </div>
                  )}
